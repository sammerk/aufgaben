## Task Name: Visual_Guessing_U1 SC #############################################
library(shiny)
library(miniUI)
library(shinyjs)
library(hrbrthemes)
...
ui <- miniPage(
  ...
  withMathJax(),
                 miniContentPanel(
                   wellPanel(
                     h4("Aufgabe:"),
                     p("In der himmlischen Weihnachtswerkstatt batteln sich die beiden Elfenteams »Jinglies« und »Sparklies«, wer mit der Fertigung welcher Geschenke schneller fertig ist. Dabei kommen die unten dargestellten Daten zustande. Schätzen Sie Cohen's U₁, $$U_3 = \\frac{\\#(Sparklies_i > \\overline{Jinglies})}{\\#Sparklies}$$ sowie $$\\text{Cohen's }d = \\frac{\\overline{Sparklies} - \\overline{Jinglies}}{\\sqrt{\\frac{s(Sparklies)^2 + s(Jinglies)^2}{2}}}$$"),
                     withSpinner(
