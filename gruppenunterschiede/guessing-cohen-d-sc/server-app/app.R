## Task Name: Visual_Guessing_d SC #############################################
library(shiny)
library(miniUI)
library(shinyjs)
library(hrbrthemes)
...
ui <- miniPage(
  ...
  withMathJax(),
                 miniContentPanel(
                   wellPanel(
                     h4("Aufgabe:"),
                     p("In der himmlischen Weihnachtswerkstatt batteln sich die beiden Elfenteams »Jinglies« und »Sparklies«, wer mit der Fertigung welcher Geschenke schneller fertig ist. Dabei kommen die unten dargestellten Daten zustande. Schätzen Sie $$\\text{Cohen's }d = \\frac{\\overline{Sparklies} - \\overline{Jinglies}}{\\sqrt{\\frac{s(Sparklies)^2 + s(Jinglies)^2}{2}}}$$"),
                     withSpinner(
...
server <- function(input, output, session) {
  ...
  output$ui_answers_task <- renderUI({
    ...
    choices <- -15:15*0.3 + d()
    choices <- round(choices[choices <= 2 & choices >= -2], 2)
