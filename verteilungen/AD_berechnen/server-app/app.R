...
server <- function(input, output, session) {
  ...
  output$prompt_task <- renderText({
    paste("Berechnen Sie die Mean Average Deviation (MAD) der Datenreihe<br>",
          "X = ", 
  ...
  correct_answers_task <- reactive({
    round2(mean(abs(data_task()-mean(data_task()))), n = 2)
  })
