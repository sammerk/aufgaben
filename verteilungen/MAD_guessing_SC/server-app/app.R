## Task Name: Visuelle_Schaetzung_MAD_SD #######################################
library(shiny)
library(miniUI)
library(shinyjs)
library(hrbrthemes)
...
ui <- miniPage(
  ...
  useShinyjs(),
                 miniContentPanel(
                   wellPanel(
                     h4("Aufgabe:"),
                     p("Schätzen Sie Mean Average Deviation (MAD) und die Standardabweichung (SD) der unten graphisch dargestellten Daten"),
...
server <- function(input, output, session) {
  ...
  output$ui_answers_task <- renderUI({
    ...
    input$new_task
    tagList(numericInput("user_MAD", "MAD", numeric(0)),
            numericInput("user_SD", "SD", numeric(0)))
  })
  ...
  output$feedback_task <- renderText({
    MAD <- round2(DescTools::MeanAD(data_task()$data), 2)
    MAD_Abweichung <- round2(100/MAD*input$user_MAD, 2)
    SD <- round2(sd(data_task()$data), 2)
    SD_Abweichung <- round2(100/SD*input$user_SD, 2)
    
    if(!is.na(input$user_MAD) & !is.na(input$user_SD)){
      feedback <-
        paste(
          "Tatsächliche MAD = ",
          MAD,
    ...
    if(is.na(input$user_MAD) | is.na(input$user_SD)){
      feedback <- 
        paste("Bitte geben Sie eine Schätzung für die MAD <i>und</i> die SD ein")
    }
  ...
  observeEvent(input$show_feedback_task, {
      ...
      sheet_append("1AZf7EQk-M2Wgej3WJXG1J2xRdql8b7Xiq0SvIiZogUo",
                   tibble(PID = ifelse(is.null(url_vars()$PID), 
                          ...
                          task_name = "Visuelle_Schaetzung_MAD_SD",
                          task_version = "guessing",
                          time = Sys.time(),
                          timezone = Sys.timezone(),
                          result = # percentage of true solutions as strings
                            paste("MAD_perc =",
                                  round2(100/round2(DescTools::MeanAD(data_task()$data), 2)*input$user_MAD, 2),
                                  "SD_perc =",
                                  round2(100/round2(sd(data_task()$data), 2)*input$user_SD, 2),
                                  sep = )
                          ),
                   sheet = 1)
     # }
